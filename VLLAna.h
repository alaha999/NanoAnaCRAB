...
public :
  ...
  TTreeReader     fReader_1718;  //reads the Common 2017/18 trigger branches
  TTreeReader     fReader_special;//reads the special branches available in few samples
  ...
  //TTreeReaderArray<Int_t> Jet_nConstituents = {fReader, "Jet_nConstituents"}; //NanoAODv8(VLL)
  TTreeReaderArray<UChar_t> Jet_nConstituents = {fReader, "Jet_nConstituents"}; //NanoAODv9
  ...
  //Special branch for year 2016
  TTreeReaderValue<Bool_t> HLT_IsoTkMu24 = {fReader_2016, "HLT_IsoTkMu24"};
  
  //Common trigger branch for year 2017 and 2018
  TTreeReaderValue<Bool_t> HLT_Ele32_WPTight_Gsf_L1DoubleEG = {fReader_1718, "HLT_Ele32_WPTight_Gsf_L1DoubleEG"};
  
  //Special branch for year 2018
  TTreeReaderValue<Bool_t> HLT_Ele32_WPTight_Gsf = {fReader_2018, "HLT_Ele32_WPTight_Gsf"};
  ...
  //LHE
  //TTreeReaderValue<Float_t> LHE_HT =                     {fReader_MC, "LHE_HT"}; //only needed for HTbinnedWJets
  
  //GenWeight
  TTreeReaderValue<Float_t> Generator_weight   = {fReader_MC, "Generator_weight"};
  //L1Prefiring
  TTreeReaderValue<Float_t> L1PreFiringWeight_Nom = {fReader_MC, "L1PreFiringWeight_Nom"};
  TTreeReaderValue<Float_t> L1PreFiringWeight_Up = {fReader_MC, "L1PreFiringWeight_Up"};
  TTreeReaderValue<Float_t> L1PreFiringWeight_Dn = {fReader_MC, "L1PreFiringWeight_Dn"};  
  //PileUp
  ...
  //special branches
  TTreeReaderValue<UInt_t> nLHEPdfWeight   = {fReader_special, "nLHEPdfWeight"};
  TTreeReaderArray<Float_t> LHEPdfWeight   = {fReader_special, "LHEPdfWeight"};
  TTreeReaderValue<UInt_t> nLHEScaleWeight = {fReader_special, "nLHEScaleWeight"};
  TTreeReaderArray<Float_t> LHEScaleWeight = {fReader_special, "LHEScaleWeight"};
  
  //LHE
  TTreeReaderValue<Float_t> LHE_HT         = {fReader_special, "LHE_HT"}; //only needed for HTbinnedWJets
  TTreeReaderValue<Float_t> LHE_Vpt        = {fReader_special, "LHE_Vpt"};//WJets samples
...
public:
  struct Hists {
    ...
    //BookHistogram()
    TH1F *nevt, *metfilter[11],*trigobj[8], *nlep[8], *ptlep[9],*etmiss;
    TH2F *trigobj2d[3];
  ...
  //float btagsf2018 (int MCsample, vector<Lepton>Jet);
  //float btagsf2017 (int MCsample, vector<Lepton>Jet);
  //float btagsf2016preVFP (int MCsample, vector<Lepton>Jet);
  //float btagsf2016postVFP (int MCsample, vector<Lepton>Jet);
...
private:
  ...
  int nEvtTotal,nEvtRan,nEvtTrigger,nEvtSkim;
  int n_4Lskim,n_3Lskim,n_2Lskim,n_1Lskim;
  float genEventsumw;
...
void VLLAna::Init(TTree *tree)
{
  ...
  if(_year == 2017 || _year==2018)
    fReader_1718   .SetTree(tree);

  //special(Only MC)
  if(((_sample<9) || (_sample>19)) && _data==0)
    fReader_special.SetTree(tree);
